...
var save_manager := preload("res://system/game/save_manager.gd").new()
...
func load_from_serialized_game(game_save: Dictionary):
...
func _input(event: InputEvent):
	if event.is_action_pressed("pause_game") and not event.is_echo() and is_in_game:
		if get_tree().paused:
